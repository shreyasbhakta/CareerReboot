 ...
 * @param {{fetchJson?: Function, providers?: Map, httpCtx?: object, onResult?: Function}} [deps]
 *   `providers`/`httpCtx` enable tier 2; omit them (as the ATS unit tests do) to
 *   get tier-1-only behavior where non-ATS entries stay `skipped`. `onResult`,
 *   if given, fires synchronously right after each entry's row is pushed —
 *   the probing loop below is already sequential (one entry at a time, "to
 *   stay gentle on rate limits"), so a caller that wants live per-company
 *   progress (the web Portals page, #background-jobs) doesn't need to wait
 *   for the whole sweep like the CLI's own end-of-run printResults() does.
 ...
 * @param {{fetchJson?: Function, onResult?: Function}} [deps] `onResult` passes
 *   straight through to verifyCompanies() — see its own doc for why.
...
// Shared by main()'s live per-company progress line and printResults()'s
// (now CLI-only, used only when a caller doesn't pass onResult) end-of-run
// dump — one line format, not two copies that could drift.
...
async function main() {
  ...
  // Probing is already one company at a time (see verifyCompanies' own doc) —
  // print as each result lands instead of waiting for the whole sweep, so a
  // 180+-company portals.yml shows real progress instead of a silent minute.
  const printLive = (r) => console.log(formatResultLine(r));
  const { results } = await verifyPortalsFile(filePath, { fetchJson, providers, httpCtx, onResult: printLive });
