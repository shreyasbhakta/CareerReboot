...
export function buildLocationFilter(locationFilter) {
  ...
  // Opt-in (default false, byte-identical prior behavior for every existing
  // config): when `allow` is explicitly set, `strict` closes the two gaps
  // that let a posting outside the selected countries through anyway —
  // (1) a blank/rolled-up location field auto-passing ("don't penalize
  // missing data" is the wrong call once the user has stated exactly which
  // locations they want), and (2) a bare "Remote" title rescuing a posting
  // that matched nothing in `allow` (remote-in-Bengaluru is not remote-in-the-
  // country the user asked for). Both leaks were deliberate, hard-won
  // leniency for the un-scoped case (see the comments above/below) — strict
  // mode only removes them when the caller has actually opted in, so the
  // default scan.mjs/scan-ats-full.mjs behavior for every config without this
  // flag is unchanged.
  ...
  return (location, url, title) => {
    ...
    if (lower === '' && hint === '') {
      // Strict + an explicit allow list → reject rather than pass: the user
      // asked for specific locations, so an unknown one does not get the
      // benefit of the doubt.
      if (strict && allow.length > 0) return false;
      // Otherwise: nothing to judge on either field → pass (don't penalize missing data).
      return true;
    }
    ...
    // Strict mode: no remote-title rescue. A bare "Remote" title says nothing
    // about WHICH country the role is remote in/eligible from — admitting it
    // here is exactly the leak strict mode exists to close (an explicit allow
    // list is the user naming the countries that matter, not "anywhere with
    // a distributed team").
...
export function formatPipelineOffer(offer) {
  ...
  // Labeled `saved:` segment (Dashboard Kanban board) — set ONLY by the web's
  // explicit "Save"/"Add to pipeline" action, never by this scanner's own bulk
  // auto-add (scan.mjs's cron-mode callers never pass `saved`), so the board's
  // Evaluate column can tell "the user chose this" from "the scanner found
  // this" — the exact distinction its own design requires. Ordered last, same
  // reasoning as note: — generic, rides on any row shape.
  return offer.saved === true ? `${line} | saved: true` : line;
}
...
// in modes/pipeline.md and expected by /career-ops pipeline. Exported so a
// caller resetting the file (clearPipeline below, or the web "Clear pipeline"
// button) writes the identical canonical text instead of a hand-copied
...
export async function appendToPipeline(offers, { pipelinePath = PIPELINE_PATH } = {}) {
  ...
  await withPipelineLock(pipelinePath, async () => {
    ...
    // Dedup against whatever pipeline.md ALREADY has (Pending or Processed) —
    // this function used to append every offer it was given unconditionally,
    // so two "Add to pipeline" clicks for the same posting (a double click, a
    // retry, a second scan surfacing it again before scan-history.tsv fully
    // reflected the first add) wrote two literal rows for the one URL. The
    // upstream seenUrls check in scan-ats-full.mjs is supposed to catch this
    // earlier, but that's a *should not offer it again* guard on the read
    // side — this is the *never write it twice* guard on the write side, and
    // a write-side bug needs a write-side fix, not a stronger read-side one.
    // Keyed by normalizeUrlForDedup, not the raw/sanitized URL text: the same
    // posting reached through two different searches routinely carries a
    // different tracking query string (utm_*, gh_src, StepStone's
    // per-request rltr, ...) — normalizeUrlForDedup strips exactly that
    // allowlisted set (the same key scan-history/in-scan dedup already use),
    // so those variants are recognized as the one URL they actually are
    // instead of writing a second pipeline.md row for it.
    const existingUrls = new Set();
    for (const line of text.split('\n')) {
      const m = line.match(/^\s*-\s*\[[ xX]\]\s*(.+)$/);
      if (!m) continue;
      const sepIdx = m[1].indexOf(' |');
      existingUrls.add(normalizeUrlForDedup((sepIdx >= 0 ? m[1].slice(0, sepIdx) : m[1]).trim()));
    }
    offers = offers.filter(o => !existingUrls.has(normalizeUrlForDedup(sanitizePipelineUrl(o.url))));
    if (offers.length === 0) return;
...
// Full reset — backs the web "Clear pipeline" button (lib/core/pipeline.ts).
// Same lock as appendToPipeline so a scan mid-write can never have its offers
// silently resurrected by (or lost to) a concurrent clear. Returns how many
// checkbox rows (Pending + Processed) were dropped, purely for the caller to
// report back — the caller owns any backup-before-clear policy, not this
// primitive, so it stays a plain reset.
export async function clearPipeline({ pipelinePath = PIPELINE_PATH } = {}) {
  return withPipelineLock(pipelinePath, () => {
    const cleared = existsSync(pipelinePath) ? (readFileSync(pipelinePath, 'utf-8').match(/^- \[/gm) || []).length : 0;
    atomicWriteFile(pipelinePath, PIPELINE_SKELETON);
    return { cleared };
  });
}

// Removes specific rows by URL — backs the web Inbox's discard ("X") button
// (#2185-follow-up). A discard is two writes, not one: this removes the row
// from pipeline.md itself (so the raw inbox file agrees with what the user
// dismissed, not just this browser's localStorage — the bug report was
// exactly that gap: a discarded posting still sat in pipeline.md, so it
// looked "gone" in one tab and reappeared everywhere else); the CALLER is
// expected to also record the URL in scan-history.tsv via appendToScanHistory
// with a non-'added' status (see shouldDedupScanHistoryRow: any status other
// than the literal 'added' is deduped PERMANENTLY, no recheck_after_days
// reopening) so a future scan's seenUrls check excludes it for good instead
// of just until the row falls out of pipeline.md. Same lock as
// appendToPipeline/clearPipeline — a scan mid-write can't race a discard.
export async function removeFromPipeline(urls, { pipelinePath = PIPELINE_PATH } = {}) {
  const urlSet = new Set((Array.isArray(urls) ? urls : [urls]).map(sanitizePipelineUrl));
  if (urlSet.size === 0) return { removed: 0 };
  return withPipelineLock(pipelinePath, () => {
    if (!existsSync(pipelinePath)) return { removed: 0 };
    const lines = readFileSync(pipelinePath, 'utf-8').split('\n');
    let removed = 0;
    const kept = lines.filter((line) => {
      // Same row shape appendToPipeline writes: "- [ ] {url} | {company} | {title}[...]".
      const m = line.match(/^\s*-\s*\[[ xX]\]\s*(.+)$/);
      if (!m) return true;
      const body = m[1];
      const sepIdx = body.indexOf(' |');
      const url = (sepIdx >= 0 ? body.slice(0, sepIdx) : body).trim();
      if (urlSet.has(url)) {
        removed++;
        return false;
      }
    ...
    if (removed > 0) atomicWriteFile(pipelinePath, kept.join('\n'));
    return { removed };
  });
}
