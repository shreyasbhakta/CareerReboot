...
// 'follow_up' added for the Dashboard Kanban's Follow-up column (templates/states.yml)
// — a row the user manually moves there is exactly as actionable as one still
// sitting in Responded/Interview; excluding it would mean a card visibly on
// the "needs a follow-up" column of the Dashboard silently never showing up
// on the /followups page, the two-competing-representations bug this whole
// state model exists to avoid.
const ACTIONABLE_STATUSES = ['applied', 'responded', 'interview', 'follow_up'];
...
export function computeUrgency(status, daysSinceApp, daysSinceLastFollowup, followupCount) {
  ...
  // Dashboard Kanban's Follow-up column (templates/states.yml `follow_up`) — the
  // user explicitly flagged this as "actively following up", which is closer to
  // an ongoing responded-style cadence than a cold, unhandled status. No thank-you
  // / initial-contact distinction applies here (unlike interview/responded), so
  // it's just the recurring re-check rhythm.
  if (status === 'follow_up') {
    if (daysSinceLastFollowup !== null) {
      return daysSinceLastFollowup >= CADENCE.responded_subsequent ? 'overdue' : 'waiting';
    }
    return daysSinceApp >= CADENCE.responded_initial ? 'overdue' : 'waiting';
  }
  return 'waiting';
}
