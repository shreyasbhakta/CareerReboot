...
export function factClaims(text, sourceNormalized = null) {
  ...
  for (const [kind, pattern] of patterns) {
    for (const match of clean.matchAll(pattern)) {
      ...
      // Split on comma-as-separator, but not a comma inside a thousands-grouped
      // number ("6,000") — that split turned "supporting 6,000+ daily
      // transactions" (4 words, correctly rejected below by the words.length
      // > 3 check) into "supporting 6" + "000+ daily transactions" (≤3 words
      // each), which slipped past isLikelyTool and got flagged as an invented
      // "tool" claim even though it's a verbatim metric from cv.md.
      const rawValues = kind === 'tool'
        ? (/^the\s+/i.test(rawText) ? [] : rawText.split(/,(?!\d)|\band\b|\bwith\b|\bin\b/i))
        : [match[1] || match[2]];
