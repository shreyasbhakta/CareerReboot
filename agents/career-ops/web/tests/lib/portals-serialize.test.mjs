...
const empty = { positive: [], negative: [], allow: [], block: [], alwaysAllow: [], blockHard: [], excludeContent: [], locationStrict: true };
...
test("content_filter.negative is written for JD-body exclusion keywords", () => {
  const doc = yaml.load(serializePortals({ ...empty, excludeContent: ["security clearance", "us citizenship"] }));
  assert.deepEqual(doc.content_filter.negative, ["security clearance", "us citizenship"]);
});

test("no excludeContent → no content_filter section", () => {
  const doc = yaml.load(serializePortals({ ...empty, positive: ["engineer"] }));
  assert.equal(doc.content_filter, undefined);
});

test("location_filter.strict is emitted only alongside a non-empty allow list", () => {
  const withAllow = yaml.load(serializePortals({ ...empty, allow: ["united states"], locationStrict: true }));
  assert.equal(withAllow.location_filter.strict, true);

  // strict is meaningless without something to be strict ABOUT — scan.mjs's
  // buildLocationFilter never even reads it when allow is empty, so emitting
  // it here would be a no-op that looks configured but isn't.
  const withoutAllow = yaml.load(serializePortals({ ...empty, blockHard: ["usa"], locationStrict: true }));
  assert.equal(withoutAllow.location_filter.strict, undefined);
});

test("locationStrict: false is not written (loose is scan.mjs's own default)", () => {
  const doc = yaml.load(serializePortals({ ...empty, allow: ["united states"], locationStrict: false }));
  assert.equal(doc.location_filter.strict, undefined);
});
