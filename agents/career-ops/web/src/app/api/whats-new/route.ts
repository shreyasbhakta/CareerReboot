...
import { MAX_SINCE_DAYS, type DiscoveredOffer } from "@/lib/explore";
...
export async function GET(req: Request) {
  ...
  // Capped at a week — same postings-age policy as Explore's scan (lib/explore.ts).
  const days = Math.min(MAX_SINCE_DAYS, Math.max(1, Number(searchParams.get("days")) || 7));
  ...
  const toOffer = (c: string[]): DiscoveredOffer | null => {
    ...
    // 'discarded' is the Inbox's own "✕" button (discardPipelineOffer) — a
    // scan-history row the user explicitly dismissed must not resurface here
    // as a "fresh match" any more than a system-skipped/expired one does.
    if (status && /skipped|expired|discarded/i.test(status)) return null;
