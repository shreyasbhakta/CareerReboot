...
import { resolveTailoredCv, resolveDefaultCv } from "@/lib/apply/cv";
...
// dir.
//
// "default=1" serves the untailored default resume (see resolveDefaultCv) and
// is otherwise independent of the three resolution paths below, in order. "n"
// (a REPORT number, e.g. from a report's
...
export async function GET(req: NextRequest) {
  ...
  const useDefault = (req.nextUrl.searchParams.get("default") ?? "").trim();

  // The untailored default resume (see resolveDefaultCv) — a distinct, self
  // contained lookup, never mixed with the report/application/company chain
  // below (that chain always resolves a job-specific TAILORED CV).
  if (useDefault) {
    const p = resolveDefaultCv();
    if (!p) return new Response("default resume not generated yet", { status: 404 });
    try {
      return servePdf(p);
    } catch {
