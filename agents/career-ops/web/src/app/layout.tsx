...
export const metadata: Metadata = {
  title: "CareerReboot",
  description: "A personal, local-first job-search command center.",
  ...
  appleWebApp: { capable: true, statusBarStyle: "black-translucent", title: "CareerReboot" },
};
...
// jarring light seam. Matches --bg (light #f8fafc / dark #0a0e17).
const THEME_SCRIPT = `(function(){try{var t=localStorage.getItem('career-ops:theme');var d=t==='dark'||(!t&&window.matchMedia('(prefers-color-scheme: dark)').matches);if(d)document.documentElement.classList.add('dark');var m=document.querySelector('meta[name="theme-color"]');if(!m){m=document.createElement('meta');m.setAttribute('name','theme-color');document.head.appendChild(m);}m.setAttribute('content',d?'#0a0e17':'#f8fafc');}catch(e){document.documentElement.classList.add('dark');}})();`;
