 ...
 */
type FilterLists = Pick<
  ExploreFilters,
  "positive" | "negative" | "allow" | "block" | "alwaysAllow" | "blockHard" | "excludeContent" | "locationStrict"
>;
...
export function seedExploreFilters(): { filters: ExploreFilters; seededFrom: string[] } {
  ...
  if (portals) {
    ...
    const cf = (portals.content_filter ?? {}) as Record<string, unknown>;
    ...
    filters.excludeContent = listFrom(cf.negative);
    // Only override the true default when the user's real portals.yml
    // explicitly opts OUT — absence keeps Explore's own default (true).
    if (lf.strict === false) filters.locationStrict = false;
    if (filters.positive.length || filters.allow.length || filters.block.length || filters.blockHard.length || filters.excludeContent.length) seededFrom.push("portals.yml");
  }
