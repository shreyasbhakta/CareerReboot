...
// A live per-offer line, streamed the moment scan-ats-full.mjs finds a match
// (--json mode) — distinguished from the final summary line by `type`.
type JsonOfferLine = { type: "offer"; offer: JsonOffer };
...
export function runDiscovery(filters: ExploreFilters, onEvent: (e: ScanEvent) => void): Promise<DiscoveredOffer[]> {
  return new Promise((resolve) => {
    ...
    let outBuf = "";
    let errBuf = "";
    // --json mode: each stdout line is its own JSON object — either a live
    // `{type:"offer",...}` emitted the moment a match is found, or (written
    // once, at the very end of a clean run) the full aggregate summary. This
    // used to be one giant blob parsed only at `close`, which meant a
    // timeout/crash mid-sweep threw away every offer already found (#3xxx) —
    // now each offer reaches `onEvent` as soon as the scanner writes it.
    let finalJson: ScanJson | null = null;

    // Workday's per-company pagination (offset-clamp recovery via sequential
    // facet-slice requests, then a second pass retrying whichever boards were
    // still truncated) is genuinely slow for large tenants — one big
    // healthcare system alone can take dozens of sequential requests. At the
    // old 230s ceiling, a default-depth scan (150 companies/ATS × 4 ATS)
    // routinely got SIGTERM'd mid-Workday-leg before it ever reached the final
    // JSON write, and --dry-run --json has no checkpoint to recover a partial
    // result from (see scan-ats-full.mjs's own note on that) — so a scan this
    // route was actively still running got reported to the user as "The
    // scanner returned no readable output," indistinguishable from a real
    // failure. This is now a background job (job-registry.ts) that survives a
    // closed tab, so a generous ceiling costs nothing but time; it exists only
    // as a safety net against a genuinely hung request, not as a work budget.
    ...
    // Shared by the live `{type:"offer"}` lines and the final summary's own
    // `offers` array (the latter is a redundant, de-duped safety net — see
    // the `close` handler below).
    const emitJsonOffer = (o: JsonOffer, sourceHint: string) => {
      const url = (o.url || "").trim();
      if (!url || seen.has(url) || !o.company || !o.title) return;
      seen.add(url);
      const source = o.source || sourceHint;
    ...
    child.stdout.on("data", (d: Buffer) => {
      if (useJson) {
        ...
        for (const p of parts) {
          if (!p.trim()) continue;
          let parsed: JsonOfferLine | ScanJson;
          try {
            parsed = JSON.parse(p);
          } catch {
            continue; // a line split mid-write by a buffer boundary — ignored, not fatal
          }
          if ((parsed as JsonOfferLine).type === "offer") {
            emitJsonOffer((parsed as JsonOfferLine).offer, `${currentAts}-full`);
          } else {
            finalJson = parsed as ScanJson; // the one non-offer line is the closing summary
          }
        }
        return;
      }
    ...
    child.on("close", () => {
      ...
      cleanupTempPortals(tempPortals);
      if (useJson) {
        // A trailing line with no newline yet (killed mid-write) never made it
        // through the `data` handler's line-splitter — try it once more here.
        if (outBuf.trim() && !finalJson) {
          try {
            const parsed = JSON.parse(outBuf) as JsonOfferLine | ScanJson;
            if ((parsed as JsonOfferLine).type === "offer") emitJsonOffer((parsed as JsonOfferLine).offer, `${currentAts}-full`);
            else finalJson = parsed as ScanJson;
          } catch {
            /* genuinely incomplete — nothing recoverable from this fragment */
          }
        }
        if (finalJson) {
          const j = finalJson;
          // Every one of these was already streamed as it was found (see
          // emitJsonOffer above); this is a redundant, de-duped safety net for
          // the rare case a line was somehow missed, not the primary path.
          if (Array.isArray(j.offers)) for (const o of j.offers) emitJsonOffer(o, `${currentAts}-full`);
          onEvent({
            kind: "summary",
          ...
          // The run was interrupted (timeout/crash) before writing its closing
          // summary, but matches WERE already streamed and are sitting in
          // `offers` — that is a partial result, not a failure, and must not
          // be reported as one: an `error` event here would make the caller
          // (route.ts) end the job on `error` and never emit `done`, discarding
          // everything already found (#3xxx). A bare summary lets the normal
          // "N roles found" outcome take over downstream.
          onEvent({ kind: "summary", companiesScanned: 0, unreachable: 0, matches: offers.length });
        } else {
          // Nothing streamed AND no closing summary — a genuine, total failure.
