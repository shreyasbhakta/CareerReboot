...
export type InboxJob = { url: string; company: string; role: string; location?: string; compensation?: string; done: boolean; postedAt?: string; saved?: boolean };
...
export function readInbox(): InboxJob[] {
  ...
  for (const line of md.split("\n")) {
    ...
    jobs.push({
      ...
      // `saved: true` — set ONLY by the web's explicit "Save"/"Add to pipeline"
      // action (see addOffersToPipeline), never by the cron scan's own bulk
      // auto-add. This is what lets the Dashboard Kanban's Evaluate column show
      // "jobs I explicitly saved" instead of the raw hundreds-strong scan
      // inbox — a row with no `saved:` label (every pre-existing row, and
      // every future cron-added one) is absent from the board, not "false".
      saved: labels.get("saved") === "true",
    });
  }
  return jobs;
}
...
/**
 * The posting URL for a row with no report to pull "**URL:**" from — i.e. one
 * created by "Proceed without evaluation" (api/pipeline/skip-evaluation),
 * where no evaluation ran and applications.md itself has no URL column at
 * all. That route embeds the URL in `notes` with this marker specifically so
 * it survives past the one write that has it in hand; every other caller
 * (Apply, "move back to Evaluate") has to be able to read it back out again.
 */
export function extractUrlFromNotes(notes: string): string | undefined {
  const m = /\[url:\s*(\S+)\]/.exec(notes);
