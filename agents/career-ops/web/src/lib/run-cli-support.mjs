...
/**
 * A CLI-announced, self-recovering retry (rate-limited, reconnecting, ...).
 * Exported so the route can surface it as a visible status — previously this
 * was silently permitted (correctly, as non-fatal) but never shown to the
 * user, so a long evaluate run that was quietly retrying looked frozen for
 * however long the retry/backoff took, up to the full per-kind timeout.
 * @param {string} line
 * @returns {boolean}
 */
export function isRetryingStderr(line) {
  return RETRYING_STDERR_RE.test(line);
}
