...
/**
 * The user's default/untailored resume — "Apply with Default Resume" (the CV
 * page's Ready-to-Apply workspace, and the Dashboard's Ready-to-Apply popup),
 * for a job the user chooses NOT to generate a tailored CV for. Distinct from
 * resolveTailoredCv above on purpose: that one is keyed to a SPECIFIC report
 * number so two applications at the same company never share a stale CV; this
 * one is the SAME single file every time, produced by compileDefaultResume()
 * (see there — a plain compile, no agent tailoring pass, so it never touches
 * cv.md/resume.tex themselves). Returns an absolute path or null if it hasn't
 * been generated yet.
 */
export function resolveDefaultCv(): string | null {
  const root = careerOpsRoot();
  const p = path.join(root, "output", "cv-default.pdf");
