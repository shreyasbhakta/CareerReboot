...
export function collectWhatsNew(rows, { cutoff, toOffer, offerLimit = DEFAULT_OFFER_LIMIT, fallbackLimit = 12 }) {
  ...
  const add = (columns, limit) => {
    // URL-dedup FIRST, on the raw column, before toOffer's status check —
    // rows are walked newest-first, so the first row seen for a URL is
    // authoritative. Marking `seen` only on a row that survives toOffer (the
    // old behavior) let an EARLIER 'added' row for the same URL slip back in
    // once a LATER row (e.g. discardPipelineOffer's 'discarded') excluded it:
    // toOffer returned null for the discard row, seen never got the URL, and
    // the reverse walk kept going until it hit the original 'added' row,
    // which passed toOffer clean and got counted — a discarded posting
    // resurfacing in "fresh matches this week" despite the discard.
    const url = columns[0];
