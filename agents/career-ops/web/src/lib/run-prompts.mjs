 ...
 */
import { CV_ENVELOPE_INSTRUCTION, CV_TEX_ENVELOPE_INSTRUCTION } from "./cv-envelope.mjs";
...
export function buildPrompt({ kind, input, memory, today, postedAt, lang, latexSource }) {
...
Target: ${input}`;
  ...
  if (kind === "pdf" && latexSource) {
    // LaTeX counterpart of the branch below, for a checkout configured with
    // cv.output_format: latex (config/profile.yml) and an actual .tex file —
    // see resolveLatexSource in pdf-paths.mjs for how "configured" is decided.
    //
    // This deliberately does NOT run modes/latex-tex.md's own pipeline: that
    // mode's extract-latex-content.mjs / patch-latex-content.mjs round-trip
    // only understands two specific macro families (\resumeSubheading-style,
    // or tabularx+itemize) and refuses anything else outright — confirmed by
    // running it, not assumed. Editing the tailored prose directly in place is
    // strictly more general (works for ANY .tex layout, since an agent
    // rewriting bullet text inside whatever macros are already there is no
    // different from it rewriting an HTML placeholder), so this is not a
    // lesser fallback for unsupported layouts — it is the one path used
    // whichever layout the file has. Same security model as the HTML branch:
    // no write tool, content arrives via envelope, backend compiles it.
    return `You are tailoring the user's own LaTeX CV (${latexSource}) for application #${input}, headless, on their machine. Apply modes/pdf.md's CONTENT rules — keyword injection, reordering by relevance, project selection, and its never-invent-a-skill rule — but edit the LaTeX prose directly in place; do NOT run modes/latex-tex.md's extractor pipeline (it only supports two specific macro layouts and this file's is not assumed to be either).
1. Read ${latexSource}, cv.md, config/profile.yml, and the evaluation report at reports/${input}-*.md (for the JD keywords + analysis).
2. Tailor ONLY the prose content — the professional summary, skill ordering/grouping, and bullet wording/ordering — using the JD's vocabulary. NEVER invent a skill, employer, metric, or claim not already in ${latexSource} or cv.md. Preserve the preamble, macros, class options, spacing commands, and every structural/formatting command EXACTLY as they are — this is a reword-and-reorder pass, not a redesign.
3. Cross-check every claim you keep or reword against cv.md; if something in ${latexSource} isn't backed by cv.md, leave it as-is rather than removing or altering it (cv.md is the fact-check source, not the sole allowed content — ${latexSource} is the user's own document).
${CV_TEX_ENVELOPE_INSTRUCTION}
4. Emit the envelope EXACTLY ONCE. The platform writes the .tex, compiles the PDF, and updates the tracker's PDF column itself, only after a confirmed successful compile. Do not submit anything anywhere.

After the envelope, end with EXACTLY one final line: VERDICT: {5 if the complete .tex envelope was emitted, else 1}/5 — {a one-line summary, ≤12 words}`;
  }
  if (kind === "pdf") {
