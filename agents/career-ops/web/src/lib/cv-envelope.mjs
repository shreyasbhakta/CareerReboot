...
// ── LaTeX counterpart (the user-owned-.tex pdf path, config/profile.yml's
// `cv.output_format: latex`) ─────────────────────────────────────────────
//
// A parallel marker pair and parser rather than a parameterized version of
// the HTML ones above: the HTML envelope's line-anchored, streaming,
// fail-closed design (exactly one envelope, first-closer-wins, complete-
// lines-only) applies unchanged to LaTeX source — a `.tex` file is exactly
// as much "one opaque blob of agent-emitted bytes" as HTML is — but forking
// the marker string is cheaper and lower-risk than threading a marker-name
// parameter through every regex, capture group and streaming-buffer edge
// case in the functions above, all of which are relied on being correct.
export const OPEN_MARK_TEX = "<<cv-tex";
export const CLOSE_MARK_TEX = "<</cv-tex>>";

const OPENER_TEX_SRC = String.raw`^<<cv-tex>>[ \t]*$`;
const CLOSER_TEX_SRC = String.raw`^<<\/cv-tex>>[ \t]*$`;
const OPENER_TEX = new RegExp(OPENER_TEX_SRC, "gm");
const CLOSER_TEX = new RegExp(CLOSER_TEX_SRC, "m");
const CLOSER_TEX_ALL = new RegExp(CLOSER_TEX_SRC, "gm");

/**
 * No page-format attribute on this opener (unlike `<<cv-html format="...">>`):
 * a LaTeX document's page size comes from its own preamble (\documentclass
 * options, \geometry{...}), which the agent is tailoring content inside of,
 * not choosing fresh — there is nothing for the backend to pick here.
 */
export const CV_TEX_ENVELOPE_INSTRUCTION =
  `Do NOT save or edit any file — the platform persists your output for you. Instead OUTPUT the complete tailored .tex source inline, between two marker lines. The first line contains only \`${OPEN_MARK_TEX}>>\` and nothing else. Then the ENTIRE .tex document — preamble untouched, only the prose/bullet content tailored — then a final line containing only \`${CLOSE_MARK_TEX}\`. Each marker appears exactly once. Emit the WHOLE document from \\documentclass to \\end{document} — never abbreviate, summarize, or write "unchanged"; the platform compiles exactly these bytes and nothing else.`;

/**
 * @typedef {Object} CvTexEnvelope
 * @property {true} ok
 * @property {string} tex - The tailored .tex source, byte-exact as emitted.
 * @property {string[]} warnings - Non-fatal issues to surface in the run log.
 */

/**
 * LaTeX counterpart to parseCvEnvelope. See that function's doc comment for
 * the shared rationale (result-not-throw, so the /api/run close handler
 * routes failure through the same honesty gate as every other pdf outcome).
 * @param {string} text
 * @returns {CvTexEnvelope | {ok: false, error: string}}
 */
export function parseCvTexEnvelope(text) {
  if (typeof text !== "string") {
    return { ok: false, error: "No agent output to read a CV envelope from." };
  }
  const normalized = text.replace(/\r\n/g, "\n");

  const openers = [...normalized.matchAll(OPENER_TEX)];
  if (openers.length === 0) {
    return { ok: false, error: "The agent emitted no <<cv-tex>> envelope, so there is no CV to compile." };
  }
  if (openers.length > 1) {
    return { ok: false, error: `Found ${openers.length} <<cv-tex>> envelopes; refusing to guess which is the real CV.` };
  }

  const opener = openers[0];
  const afterOpener = normalized.slice(opener.index + opener[0].length);
  const closer = CLOSER_TEX.exec(afterOpener);
  if (!closer) {
    return { ok: false, error: "The <<cv-tex>> envelope was never closed — the CV output is incomplete." };
  }

  const tex = afterOpener.slice(0, closer.index).replace(/^\n/, "").replace(/\n$/, "");
  if (!tex.trim()) {
    return { ok: false, error: "The <<cv-tex>> envelope was empty — no CV was tailored." };
  }
  // \end{document} is what distinguishes a whole document from one the agent
  // cut off mid-emission — the realistic failure when emitting a full CV inline.
  if (!/\\end\{document\}/.test(tex)) {
    return { ok: false, error: "The CV was cut off before its \\end{document} — the output is incomplete." };
  }

  return { ok: true, tex, warnings: [] };
}

/**
 * Streaming companion to parseCvTexEnvelope, mirroring createCvEnvelopeFilter
 * exactly (see that function's doc comment) with the `<<cv-tex>>` markers.
 * @returns {{push: (chunk: string) => string, flush: () => string, result: () => (CvTexEnvelope | {ok: false, error: string})}}
 */
export function createCvTexEnvelopeFilter() {
  let raw = "";
  let pending = "";
  let carry = "";
  let inBody = false;

  const couldBecomeTexMarker = (line) =>
    OPEN_MARK_TEX.startsWith(line) || CLOSE_MARK_TEX.startsWith(line) ||
    line.startsWith(OPEN_MARK_TEX) || line.startsWith(CLOSE_MARK_TEX);
  ...
  return {
    push(chunk) {
      ...
      for (;;) {
        if (!inBody) {
          const opener = completeMarker(OPENER_TEX, pending);
          if (opener) {
          ...
          const cut = couldBecomeTexMarker(pending.slice(tailStart)) ? tailStart : pending.length;
          display += pending.slice(0, cut);
          pending = pending.slice(cut);
          return display;
        }
        const closer = completeMarker(CLOSER_TEX_ALL, pending);
        if (closer) {
