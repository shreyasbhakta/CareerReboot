...
type ExploreCtx = {
  ...
  discardOffer: (offer: DiscoveredOffer) => void;
...
// A scan is now a background job (job-registry.ts / #background-jobs): the
// server keeps walking the ATS network even after the browser closes
// entirely — the laptop sleeping mid-scan is exactly the case this exists
// for. localStorage (not sessionStorage, which a real tab close wipes) so
// reopening the browser later — a new tab, even a new window — finds it;
// two tabs both reconnecting to the same job is harmless (job-registry.ts
// supports any number of listeners), so there's no reason to scope it tighter.
const RUNNING_SCAN_KEY = "career-ops:explore-running-scan";
function persistRunningScan(jobId: string) {
  try { localStorage.setItem(RUNNING_SCAN_KEY, jobId); } catch { /* best-effort */ }
}
function clearRunningScan() {
  try { localStorage.removeItem(RUNNING_SCAN_KEY); } catch { /* best-effort */ }
}
function readRunningScan(): string | null {
  try { return localStorage.getItem(RUNNING_SCAN_KEY); } catch { return null; }
}
...
export function ExploreProvider({ children }: { children: React.ReactNode }) {
  ...
  // Shared by discover() (the connection that starts a scan) and reconnectScan()
  // (a reopened tab picking one back up) — the event handling is identical
  // either way, which is what makes resuming after a closed tab possible at
  // all (#background-jobs): the backend scan kept running regardless of who,
  // if anyone, was reading its output.
  const consumeScanStream = useCallback(async (reader: ReadableStreamDefaultReader<Uint8Array>, acc: DiscoveredOffer[]) => {
    let sawError = "";
    ...
    for (;;) {
      ...
      while ((nl = buf.indexOf("\n")) >= 0) {
        ...
        switch (ev.kind) {
          case "jobId":
            // Persisted immediately (not just on settle) so a reload mid-scan
            // still has something to reconnect to (#background-jobs).
            persistRunningScan(ev.id);
            break;
          case "atsStart":
            setPhase("scanning");
    ...
    return { sawError, sawScannerMissing, companiesScannedAcc, capHitAcc, datasetIssueAcc, droppedNoDateAcc };
  }, []);

  // The settle-phase logic shared by discover() and reconnectScan() once their
  // stream ends, one way or another.
  const finishScan = useCallback(
    (acc: DiscoveredOffer[], outcome: { sawError: string; sawScannerMissing: boolean; companiesScannedAcc: number; capHitAcc: boolean; datasetIssueAcc: boolean; droppedNoDateAcc: number }, broad: boolean) => {
      clearRunningScan();
      ...
      if (acc.length > 0) {
      ...
      } else if (outcome.sawError) {
        setError(outcome.sawError);
        setScannerMissing(outcome.sawScannerMissing);
        setPhase("failed");
      } else if (outcome.capHitAcc || outcome.datasetIssueAcc || outcome.droppedNoDateAcc > 0 || outcome.companiesScannedAcc === 0) {
        ...
        setPhase(broad ? "empty-current" : "empty-loose");
      }
    },
    [],
  );
  ...
  const discover = useCallback(async () => {
    ...
    let outcome = { sawError: "", sawScannerMissing: false, companiesScannedAcc: 0, capHitAcc: false, datasetIssueAcc: false, droppedNoDateAcc: 0 };
    try {
      ...
      if (!r.ok) {
        ...
        outcome.sawScannerMissing = isScannerMissing(d);
        outcome.sawError = d.error || (outcome.sawScannerMissing ? "The scanner isn't available." : `Discovery failed (${r.status}).`);
      } else if (!r.body) {
        outcome.sawError = "No response stream.";
      } else {
        outcome = await consumeScanStream(r.body.getReader(), acc);
      }
    } catch (e) {
      outcome.sawError = e instanceof Error ? e.message : "stream error";
    }

    finishScan(acc, outcome, isBroadSearch(f));
  }, [consumeScanStream, finishScan]);

  // Picked up on mount when a scan was still running when this tab last
  // closed/reloaded (#background-jobs) — the backend job kept walking the ATS
  // network the whole time, so this just resumes watching it instead of
  // showing a completed (or still-running) scan's results as "idle".
  const reconnectScan = useCallback(
    async (jobId: string) => {
      ...
      setScannerMissing(false);
      setSources({});
      setStatus("Reconnecting to your scan…");

      const acc: DiscoveredOffer[] = [];
      let outcome = { sawError: "", sawScannerMissing: false, companiesScannedAcc: 0, capHitAcc: false, datasetIssueAcc: false, droppedNoDateAcc: 0 };
      try {
        const r = await fetch(`/api/explore/stream?id=${encodeURIComponent(jobId)}`);
        if (!r.ok) {
          // Most likely the server restarted since — the scan is genuinely
          // gone, not failed. Fail quietly back to idle rather than surfacing
          // an alarming error for a run the user didn't just click.
          clearRunningScan();
          runningRef.current = false;
          setPhase("idle");
          setStatus("");
        ...
        if (r.body) outcome = await consumeScanStream(r.body.getReader(), acc);
      } catch (e) {
        outcome.sawError = e instanceof Error ? e.message : "stream error";
      }
      finishScan(acc, outcome, isBroadSearch(filtersRef.current));
    },
    [consumeScanStream, finishScan],
  );
  ...
  const addToPipeline = useCallback(async (list: DiscoveredOffer[]) => {
    ...
    try {
      ...
      if (d.added && d.added > 0) {
        const addedUrls = new Set(fresh.map((o) => o.url));
        setAdded((s) => new Set([...s, ...addedUrls]));
        // Saved jobs move to the Dashboard, not stay duplicated here — the
        // whole point of "Add to pipeline" is to keep the two boards
        // organized rather than showing the same job in both places
        // indefinitely. Same removal call discardOffer already uses.
        setOffers((o) => o.filter((x) => !addedUrls.has(x.url)));
  ...
  // "✕" on a result card — the bug report was that Explore's own results had
  // no way to say "not interested" at all, only Add/Evaluate. Removes it from
  // THIS list immediately (optimistic — matches addToPipeline's own instant
  // feedback) and durably server-side via discardPipelineOffer (removes any
  // pipeline.md row + records scan-history.tsv 'discarded', permanently
  // excluded from future scans — see that function's own doc).
  const discardOffer = useCallback((offer: DiscoveredOffer) => {
    setOffers((o) => o.filter((x) => x.url !== offer.url));
    fetch("/api/pipeline/discard", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: offer.url, company: offer.company, title: offer.title, location: offer.location, source: offer.source || offer.ats }),
    }).catch(() => {
  ...
  // Reconnect to a scan that was still running when this tab last closed or
  // reloaded (#background-jobs) — checked BEFORE the settled-snapshot rehydrate
  // below so it wins: reconnectScan flips runningRef.current synchronously
  // (before its first await), so if a running scan is found, the rehydrate
  // effect's own `if (runningRef.current) return;` guard skips restoring a
  // stale settled snapshot on top of it.
  useEffect(() => {
    const jobId = readRunningScan();
    if (jobId) void reconnectScan(jobId);
    // Deliberately only on mount — reconnectScan/readRunningScan are stable
    // enough for this one-shot check.
  ...
  const value = useMemo(
    () => ({
      ...
      discover, loadFresh, addToPipeline, discardOffer, applyPatch, reset,
      mode, setMode, aiIntent, setAiIntent, discoverAI, aiTrace, aiCost,
    }),
    [filters, setFilters, initFilters, phase, offers, sources, matchCount, companiesScanned, companiesAvailable, capHit, droppedNoDate, status, partial, error, scannerMissing, added, adding, discover, loadFresh, addToPipeline, discardOffer, applyPatch, reset, mode, setMode, aiIntent, discoverAI, aiTrace, aiCost],
  );
