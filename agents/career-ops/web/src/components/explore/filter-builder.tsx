...
// Capped at a week — see MIN_SINCE_DAYS/MAX_SINCE_DAYS in lib/explore.ts. A
// listing older than that is rarely still open, and a week of daily-scanned
// volume is already plenty.
const RECENCY = [
  { label: "10hr", days: 10 / 24 },
  { label: "24hr", days: 1 },
  { label: "2d", days: 2 },
  { label: "3d", days: 3 },
  { label: "5d", days: 5 },
...
function KeywordField({
  ...
  return (
    <div className={cn("co-fb__field border border-border bg-surface/40 focus-within:border-brand/40 transition-colors")}>
      {values.map((v) => {
        // portals.yml's word:/stem: anchoring prefixes (see its own title_filter
        // docs) are a real, meaningful distinction there, but rendered as raw
        // chip text ("word:Intern") they read as broken/leaked internals to
        // anyone who's never opened that file — display the plain keyword,
        // keep matching/removal on the full underlying value.
        const display = v.replace(/^(word|stem):/, "");
        return (
          <span key={v} className={cn("co-fb__chip", tone === "inc" ? "inc" : "border-border bg-surface-hover text-muted")}>
            ...
            {display}
            <button type="button" aria-label={`Remove ${display}`} onClick={() => onChange(values.filter((x) => x !== v))}>
              <X className="size-3" />
            </button>
          </span>
        );
...
export function FilterBuilder({
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <Label hint="checked against the posting text, not just the title — works for Greenhouse/Lever/Ashby">
          Exclude if the posting mentions
        </Label>
        <KeywordField
          values={filters.excludeContent}
          tone="exc"
          placeholder="security clearance, US citizenship, active clearance…"
          onChange={(v) => set({ excludeContent: v })}
        />
      </div>
        ...
        <div className="space-y-3 rounded-xl border border-border bg-surface/30 p-3">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div>
              ...
              {filters.allow.length > 0 && (
                <label className="mt-1.5 flex items-center gap-1.5 text-[11px] text-muted">
                  <input
                    type="checkbox"
                    checked={filters.locationStrict}
                    onChange={(e) => set({ locationStrict: e.target.checked })}
                    className="accent-brand"
                  />
                  Strict — reject postings with an unclear location instead of letting them through
                </label>
              )}
            </div>
            <div>
