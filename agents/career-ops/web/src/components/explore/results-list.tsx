...
export function ResultsList({ offers }: { offers: EnrichedOffer[] }) {
  const { companiesScanned, partial, addToPipeline, discardOffer, added, mode } = useExplore();
  const isAi = mode === "ai";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {view.map((o) => (
          <DiscoveryCard key={o.url} offer={o} inPipeline={o.inPipeline} evaluatedN={o.evaluatedN} onDiscard={() => discardOffer(o)} />
        ))}
      </div>
