...
export function DiscoveryCard({
  ...
  inPipeline: boolean;
  evaluatedN?: string;
  /** Present on every surface that renders this card now (Today's "Fresh matches",
   *  Explore's results, AI hunt) — there was previously no way to say "not
   *  interested" here at all, only "Add to pipeline" (which is exactly the
   *  bug report: cards with no discard button). Optional only so a caller
   *  mid-render-cycle can omit it rather than wire a no-op. */
  onDiscard?: () => void;
}) {
  ...
  return (
    <div className="co-rise group flex min-w-0 flex-col gap-2.5 rounded-xl border border-border bg-surface/40 p-3.5 text-left transition-all hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-sm">
      <div className="flex items-start gap-3">
        ...
        {onDiscard && !working && !doneEval && !evaluatedN && (
          <button
            type="button"
            onClick={onDiscard}
            title="Discard — won't show up again in a future scan"
            aria-label="Discard"
            className="-m-1 inline-flex shrink-0 items-center justify-center rounded p-1 text-faint transition-colors hover:bg-surface-hover hover:text-foreground max-sm:min-h-[44px] max-sm:min-w-[44px]"
          >
            <X className="size-4" />
          </button>
        )}
