...
import { paramsToFilters, paramsToAi, MAX_SINCE_DAYS, type ExploreFilters } from "@/lib/explore";
...
export function ExplorerView({
  ...
  // Default OPEN once results exist: the filters the user just searched with
  // are exactly what they need to see (and adjust) to narrow a noisy result
  // set — hiding them behind a click every time defeats the "refine" use case.
  ...
  const enriched: EnrichedOffer[] = useMemo(
    () =>
      offers.map((o) => {
        ...
        const ev = findEvaluatedApplication({ company: o.company, role: o.title }, appsSnapshot);
  ...
  const isAi = mode === "ai";
  if (running) return isAi ? <AiHuntView cliName={cli.name} /> : <DiscoveringState offers={enriched} />;
        ...
        <>
          ...
          {phase === "empty-current" && (
            <EmptyState
              ...
              note={scanNote}
              onRerun={() => {
                setFilters({ ...filters, sinceDays: Math.max(filters.sinceDays, MAX_SINCE_DAYS) });
                void discover();
              }}
              rerunLabel={`Look back ${MAX_SINCE_DAYS} days`}
            />
          )}
          {phase === "empty-loose" && (
            <EmptyState
              ...
              note={scanNote}
              onRerun={() => {
                setFilters({ ...filters, sinceDays: MAX_SINCE_DAYS, block: [], allow: [] });
                void discover();
              }}
              rerunLabel={`Widen to ${MAX_SINCE_DAYS} days · clear location`}
            />
          )}
          {phase === "degraded" && (
            <DegradedCard
