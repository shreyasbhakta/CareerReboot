...
function describePage(p: string): string {
  if (p === "/") return "Dashboard — the Kanban board of the user's saved jobs (Evaluate → Ready to Apply → Applied → Follow-up → Rejected/Ghosted → Success).";
  if (p === "/pipeline") return "Pipeline — the full historical applications table + the inbox of pending job URLs (superseded by the Dashboard Kanban for day-to-day use, but still the complete record).";
