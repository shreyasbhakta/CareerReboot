...
type ApplyCtx = {
  ...
  error: string;
  open: (url: string, opts?: { prefill?: boolean; company?: string; n?: string; from?: string; useDefaultCv?: boolean }) => Promise<void>;
...
export function ApplyProvider({ children }: { children: React.ReactNode }) {
  ...
  // Set once at open() time from the explicit "Apply with Default Resume"
  // choice — never inferred, never falls back, so fill() always knows exactly
  // which resume the user picked (see fill/route.ts's useDefaultCv contract).
  const useDefaultCvRef = useRef<boolean>(false);
  ...
  const open = useCallback(async (u: string, opts?: { prefill?: boolean; company?: string; n?: string; from?: string; useDefaultCv?: boolean }) => {
    const gen = ++generation.current;
    ...
    nRef.current = opts?.n ?? "";
    setFrom(opts?.from ?? "");
    useDefaultCvRef.current = !!opts?.useDefaultCv;
  ...
  const fill = useCallback(async () => {
    ...
    setSteps([]);
    try {
      const r = await fetch("/api/apply/fill", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: sessionId.current, answers, fields, handoff: true, company: companyRef.current, application: nRef.current, useDefaultCv: useDefaultCvRef.current }) });
      const d = await r.json();
