...
export function ConfigForm() {
  ...
  useEffect(() => {
    fetch("/api/clis")
      ...
      .then((d) => {
        ...
        setClis(list);
        // Only ever show a CLI as selected here if it's actually persisted, or
        // there's exactly one installed option (auto-persist that one too — an
        // unambiguous choice shouldn't need a manual save). With 2+ installed
        // CLIs and nothing saved yet, leave the picker unselected: highlighting
        // one without saving it is what caused this page to look configured
        // when `readSavedCliId()` was still null (#the "Workers" panel bug).
        setCliId((prev) => {
          if (prev) return prev;
          if (readSavedCliId()) return readSavedCliId()!;
          const only = list.filter((c) => c.installed);
          if (only.length === 1) {
            persistCliId(only[0].id);
            return only[0].id;
          }
          return "";
        });
  ...
  // Selecting a CLI takes effect immediately — no separate "Save config" step
  // for this choice. That button still exists for Appearance/cadence below,
  // but requiring it here too is exactly the two-step gap that caused a CLI
  // to look chosen while nothing was actually persisted.
              ...
              <div className="space-y-2">
                {clis.map((c) => {
                  ...
                  return (
                    ...
                    >
                      {/* Radio-style: filled = the one actually in use, outline = installed
                          but not chosen, dashed = not installed. Selecting a row IS the save —
                          this icon is the only status indicator, so it must never show "chosen"
                          without persistCliId having actually run (see selectCli above). */}
                      {selected ? (
                      ...
                      >
                        ...
                        {selected && <span className="text-xs font-medium text-emerald-500">Active</span>}
                      </button>
                      {c.installed ? (
