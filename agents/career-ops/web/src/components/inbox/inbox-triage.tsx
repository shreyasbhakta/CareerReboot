...
export function InboxTriage({ inbox }: { inbox: InboxJob[] }) {
  ...
  // Discard: hides instantly (client-side, undoable for 5s — unchanged UX),
  // then makes it durable server-side once the undo window closes. Discarding
  // used to be a localStorage-only hide, so the posting stayed in
  // data/pipeline.md and could resurface in a fresh scan or on another
  // browser — that gap is exactly what the delayed call below closes (see
  // discardPipelineOffer's own doc for why it's two writes, not one).
  const skip = (job: InboxJob) => {
    ...
    const discardTimer = setTimeout(() => {
      fetch("/api/pipeline/discard", {
        ...
        body: JSON.stringify({ url: job.url, company: job.company, title: job.role, location: job.location }),
      }).catch(() => {
        /* best-effort — the client-side hide still holds even if this fails */
      });
    }, 5000);
    setUndo({
