...
// The one shape an offer takes on stdout in --json mode, whether it arrives as
// a live `{type:"offer",...}` line during the sweep or inside the final
// summary object's `offers` array. Shared so the two paths can never drift.
function shapeOfferForJson(o) {
  return {
    company: o.company,
    title: o.title,
    url: o.url,
...
async function main() {
  ...
  const processJobs = async (jobs, sourceName, provider, companySlug) => {
    for (const job of jobs) {
      ...
      // Stream it now rather than batching to the end (#3xxx): a 500-company
      // sweep can run many minutes, and the old "one JSON blob at the very
      // end" contract meant a timeout/crash anywhere in that window lost every
      // match already found, with nothing recoverable on stdout. The
      // per-offer blacklist check is applied here (a single-item call into the
      // SAME filterBlacklistedOffers used for the final aggregate below) so a
      // do-not-apply company never flashes onto the caller's screen even
      // momentarily. This does not double-count filteredBlacklist/
      // annotatedBlacklisted — those totals still come solely from the one
      // batch call over newOffers at the end of main().
      if (opts.json) {
        const { offers: streamKept } = filterBlacklistedOffers([offer], blacklist, { includeBlacklisted: opts.includeBlacklisted });
        if (streamKept.length) {
          process.stdout.write(JSON.stringify({ type: 'offer', offer: shapeOfferForJson(streamKept[0]) }) + '\n');
        }
      }
    }
  };
  ...
  if (opts.json) {
    process.stdout.write(JSON.stringify({
      ...
      saved,
      // Every one of these was already streamed as a `{type:"offer"}` line
      // during the sweep (see processJobs above) — this array is kept as a
      // complete, backward-compatible snapshot for a caller that only reads
      // stdout at close, and costs nothing extra: a caller that already
      // streamed offers de-dupes by URL and simply skips them here.
      offers: offers.map(shapeOfferForJson),
    }) + '\n');
    return;
  }
